...
Enable: **ShieldPrompt → System Prompt Protection → ON**. No additional configuration required — Protector Plus detects system-prompt leakage in LLM responses.
...
A recommended starting configuration is to enable: LLM, Vector, Content Moderation, and System Prompt Protection guardrails. Run for 1–2 weeks in **Monitor** mode while tuning thresholds. Switch to **Block** for guardrails whose false-positive rate is acceptable for the domain.
